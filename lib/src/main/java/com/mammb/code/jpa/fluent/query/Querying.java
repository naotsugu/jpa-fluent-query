...
public interface Querying<E, R extends RootAware<E>, U> extends CreateQuery<E, R, U> {
    ...
    private static <E, R extends RootAware<E>, U> Querying<E, R, U> of(
        ...
        return new Querying<>() {
            @Override
            public Querying<E, R, U> filter(Filter<E, R> f) {
                return Querying.of(rootSource(), mapper(), filter().and(f), sorts(), hints());
            }
            @Override
            public Querying<E, R, U> sorted(Sort<E, R> sort) {
                return Querying.of(rootSource(), mapper(), filter(), sorts().and(sort), hints());
            }
            @Override
            public Querying<E, R, U> sorted(Sort<E, R> sort1, Sort<E, R> sort2) {
                return Querying.of(rootSource(), mapper(), filter(), sorts().and(sort1).and(sort2), hints());
            }
            @Override
            public Querying<E, R, U> sorted(Sort<E, R> sort1, Sort<E, R> sort2, Sort<E, R> sort3) {
                return Querying.of(rootSource(), mapper(), filter(), sorts().and(sort1).and(sort2).and(sort3), hints());
            }
            @Override
            public Querying<E, R, U> sorted(Sort<E, R> sort1, Sort<E, R> sort2, Sort<E, R> sort3, Sort<E, R> sort4) {
                return Querying.of(rootSource(), mapper(), filter(), sorts().and(sort1).and(sort2).and(sort3).and(sort4), hints());
            }
            @Override
            public Querying<E, R, U> distinct() {
                return Querying.of(rootSource(), mapper().distinct(true), filter(), sorts(), hints());
            }
            @Override
            public Querying<E, R, U> unDistinct() {
                return Querying.of(rootSource(), mapper().distinct(false), filter(), sorts(), hints());
            }
            @Override
            public <Y> Querying<E, R, Y> map(Mapper<E, R, Y> mapper) {
                return Querying.of(rootSource(), mapper, filter(), sorts(), hints());
            }
            @Override
